...
export function AdminPanel({
  ...
  const { data: submissions = [], isLoading } = useQuery(
    pendingMapSubmissionsQueryOptions(mapSlug),
  )
  ...
  const approveMutation = useMutation({
    ...
    onMutate: async (submissionId) => {
      ...
      const previous =
        queryClient.getQueryData<MapSubmission[]>(pendingQueryKey)
      queryClient.setQueryData<MapSubmission[]>(
        pendingQueryKey,
    ...
    onSettled: (result) => {
      ...
      if (result?.locationId) {
        ...
        queryClient.invalidateQueries({
          queryKey: ['mapPhotos', result.locationId],
        })
      }
    },
  })
  ...
  const rejectMutation = useMutation({
    mutationFn: (submissionId: string) =>
      rejectSubmission({ data: { submissionId } }),
    onMutate: async (submissionId) => {
      await queryClient.cancelQueries({ queryKey: pendingQueryKey })
      const previous =
        queryClient.getQueryData<MapSubmission[]>(pendingQueryKey)
  ...
  if (isLoading) {
    return (
      <p
        data-testid="pending-submissions-loading"
  ...
  if (submissions.length === 0) {
    return (
      <div className="rounded-xl border border-[var(--border)] bg-[var(--surface)] p-4">
        <p
          data-testid="no-pending-submissions"
          className="text-sm text-[var(--text-muted)]"
        >
  ...
  return (
    <div className="space-y-3">
      <h3
        data-testid="pending-submissions-heading"
        ...
        >
          <div className="flex items-start justify-between gap-2">
            <div>
              ...
              {sub.notes && (
                <p className="m-0 mt-1 text-xs text-[var(--text-muted)] italic">
                  {sub.notes}
                </p>
              )}
              {sub.submitter_name && (
