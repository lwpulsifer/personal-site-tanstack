...
describe('BookEditor ISBN lookup', () => {
  ...
  it('looks up an ISBN-13 and fills in empty title/author fields', async () => {
    vi.mocked(fetch).mockResolvedValue({
      ok: true,
      json: async () => ({
        'ISBN:9780593135204': { title: 'Project Hail Mary', authors: [{ name: 'Andy Weir' }] },
      }),
    } as Response)

    const user = userEvent.setup()
    withQueryClient(<BookEditor initial={{}} onClose={noop} onSaved={noop} onDeleted={noop} />)

    await user.type(screen.getByTestId('book-isbn-input'), '9780593135204')

    await waitFor(
      () =>
        expect((screen.getByTestId('book-title-input') as HTMLInputElement).value).toBe(
          'Project Hail Mary',
        ),
      { timeout: 3000 },
    )
    expect((screen.getByTestId('book-author-input') as HTMLInputElement).value).toBe('Andy Weir')
  ...
  it('shows an error message when the ISBN is not found', async () => {
    ...
    await waitFor(
      () => expect(screen.getByTestId('isbn-lookup-error').textContent).toContain('No book found'),
      { timeout: 3000 },
    )
  })
  ...
  it('shows an error message when the fetch itself fails', async () => {
    ...
    await waitFor(
      () => expect(screen.getByTestId('isbn-lookup-error').textContent).toContain("Couldn't look up"),
      { timeout: 3000 },
    )
  })
  ...
  it('does not overwrite a manually entered title', async () => {
    ...
    expect((screen.getByTestId('book-title-input') as HTMLInputElement).value).toBe('My Custom Title')
  })
})
