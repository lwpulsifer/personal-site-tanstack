...
describe('AdminPanel', () => {
  it('renders pending submissions', async () => {
    ...
    expect(await screen.findByText('Pending Submissions (2)')).toBeTruthy()
  ...
  it('calls onSelectSubmission when a submission is clicked', async () => {
    ...
    renderWithProvider(
      <AdminPanel mapSlug="lions" onSelectSubmission={onSelect} />,
    )
  ...
  it('highlights the selected submission', async () => {
    const { container } = renderWithProvider(
      <AdminPanel mapSlug="lions" selectedSubmissionId="sub-1" />,
    )
